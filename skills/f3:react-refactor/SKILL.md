...
Locate and read the shared rules file via Glob for `**/skills/_shared/f3-react-rules.md`. These 8 rules (S1–S8) are the shared React/Next.js code quality baseline used across all `/f3:*` skills - named exports, server components, `next/image`, strict TypeScript, zero `eslint-disable`, ESLint `--max-warnings 0`, Prettier compliance, and business logic in hooks.
