...
- `/f3:sweep` -- all open PRs, full pipeline
- `/f3:sweep --author pstaylor-patrick` -- only my PRs
- `/f3:sweep --label "needs review"` -- only PRs with that label
- `/f3:sweep --stages review,qa` -- skip resolve-threads
- `/f3:sweep --cap 2 --dry-run` -- preview what would be swept
- `/f3:sweep --no-drafts` -- skip draft PRs
...
Agent:
  ...
  prompt: |
    ...
    Run: Skill("f3:resolve-threads", "$PR_NUMBER")
    ...
    Stage 2: Code Review
    Run: Skill("f3:code-review", "$PR_NUMBER")
...
Create a markdown report at `/tmp/f3-sweep-$(date +%Y%m%d-%H%M%S).md`:
