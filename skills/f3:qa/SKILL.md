...
SKILL_LINK="$HOME/.claude/commands/f3:qa.md"
...
```bash
node "$CDP_BRIDGE" stream --port $CDP_PORT --output /tmp/f3-qa-cdp-$(date +%s).jsonl &
CDP_STREAM_PID=$!
```
...
```bash
QA_EVIDENCE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/f3-qa-XXXXXX")
```
...
- **All pass:** "All tests passed. Run `/f3:qa` (without `--preflight`) to save evidence and post results to the PR."
- **Any fail:** "Failures detected. Fix the issues above, then re-run `/f3:qa --preflight` to verify before posting."
