...
This is a lightweight alternative to `/f3:qa` -- terminal commands only, no browser automation.
...
**Title:** Derive from the full commit log. Keep it under 70 characters. If the branch has a single focus, use that. If multiple distinct changes exist, use a summary title (e.g., "Add /f3:push skill with auto-commit and PR refresh").
