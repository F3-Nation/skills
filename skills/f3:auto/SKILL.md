...
Write `.f3-auto-progress.json` at the repo root after each phase completes. Add the file to `.git/info/exclude` so it is never committed.
...
**Recovery:** If `.f3-auto-progress.json` exists when the skill starts, read it. Resume from the first phase NOT in `completed` or `skipped`. Do NOT re-run the interview.
...
Skills to invoke:
  ...
  - f3:slop --auto
  - f3:react-refactor (if TSX changed)
  - f3:code-review --preflight (review loop)
  - f3:push
  - f3:qa (if user-facing surface)
  - f3:code-review --autofix (final review)
```

Print the plan to the terminal. **Do NOT ask for approval.** Proceed immediately into autonomous execution.

Write `.f3-auto-progress.json` with state `implement`. Add to `.git/info/exclude`:

```bash
grep -qxF '.f3-auto-progress.json' .git/info/exclude 2>/dev/null || echo '.f3-auto-progress.json' >> .git/info/exclude
```
...
**If a Figma URL was provided:** invoke `Skill("f3:figma", "<url>")` for UI implementation.
...
If warranted: `Skill("f3:react-refactor")`
...
1. Run: `Skill("f3:code-review", "--preflight")`
...
**Sweep shortcut:** If the code-review skill supports `--sweep`, use `Skill("f3:code-review", "--preflight --sweep")` instead of the manual loop. The sweep mode internally handles min/max rounds.
...
If applicable: `Skill("f3:qa", "--preflight")`
...
```
Skill("f3:code-review", "--autofix <PR_NUMBER>")
```
...
2. Re-run: `Skill("f3:push")` to push fixes and update PR
