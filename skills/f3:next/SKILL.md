...
PR #{PR_NUMBER} is up and CI is green. Get it reviewed.
  ...
  /f3:code-review {PR_NUMBER}
```

**Why:** Code review is the gate between "code works" and "code is ready." `/f3:code-review` validates findings in isolated worktrees so every reported issue has a proven fix.
...
**Why:** QA is the last gate before merge. `/f3:qa` synthesizes a test plan from the PR context and executes it via browser automation.
